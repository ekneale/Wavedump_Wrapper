...
//++++++++++++++ Build the model ++++++++++++++++++++
RooAddPdf* makePMTPDF(RooRealVar* counts,double pmval, double psval, double psval2,  double mval, double sval, double f1peval){// counts,pedPeak,sigPeak-pedPeak,sped,ssignal,Ratio,valleyPos

  std::cout << pmval << " " << psval << " "  << mval << " "  << sval  << " fraction is "  <<f1peval << std::endl;
  ...
  RooRealVar* fped = new RooRealVar("fped","fped",0.95,0.9,1 );//TODO changed according to pedestal fraction from new charge histos
 ...
 // first construct the Gaussians
  ...
  RooAddPdf* smodel = new RooAddPdf("smodel","smodel",RooArgList(*pedpdf,*gauss,*gauss2,*gauss3), RooArgList(*frac_ped1,*frac_pe1,*frac_pe2,*frac_pe3));
...
int main(int argc,char **argv){	
  ...
  for (int i=0;i<125; i++){
    ...
    for (int r=0;r<5;r++){ 
      ...
      char root_name[30];
      sprintf(root_name, "hQ_Filter_Peak_Run_%d_PMT_%d_Loc_%d_HV_%d",run,pmt,loc,hv);
